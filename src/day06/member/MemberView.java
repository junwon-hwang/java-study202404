...
public class MemberView {
    ...
    boolean exitProgram(){
        ...
        if(exit.equals("y")){
            ...
            System.out.println("프로그램 종료를 취소 합니다.");
            return false;
        }

        }
        // 이메일 입력받고 찾은 회원 정보를 출력
    ...
    // 수정 대상의 이메일을 입력받고 조회에 성공하면 패스워드를 수정해준다.
    public void updatePassword() {
        String inputEmail = si.input("# 수정할 대상의 이메일을 입력하세요. \n>>");
        // 이메일이 일치하는 회원이 있는지 조회
        Member foundMember = mr.findMemberByEmail(inputEmail);
        if(foundMember != null){
            // 비번 수정
            System.out.println(foundMember.memberName+"의 비밀번호를 변경합니다!");
    ...
    public void deleteMember() {
        String inputEmail = si.input("# 삭제할 대상의 이메일을 입력하세요. \n>>");
        // 이메일이 일치하는 회원이 있는지 조회
        Member foundMember = mr.findMemberByEmail(inputEmail);
        if(foundMember != null){
            // 삭제 진행
